...
public:
	UiHistoryList() { _btn = 0; nulldata = false;}
public:
    ...
    void SetDataNULL() { nulldata = true; }
    bool isDataNull() { return nulldata; }
private:
	UiButton* _btn;
    bool nulldata;
};
...
void UiHistoryList::DrawItem(HDC hdcDst, int nIndex, RECT* prcItem, RECT *prcWin, RECT *prcUpdate){
    if(nulldata){
        return UiList::DrawItem(hdcDst,nIndex,prcItem,prcWin,prcUpdate);
    }
...
UI_HistoryWnd::~UI_HistoryWnd(){
    if(m_pListHistory) delete m_pListHistory;
}
...
BOOL UI_HistoryWnd::OnInitDialog() {
    ...
    m_pListHistory = new UiHistoryList;
    m_pListHistory->SetPos(0, y, GetWidth(), GetHeight() - y - MZM_HEIGHT_TOOLBARPRO);
	m_pListHistory->SetID(MZ_IDC_LIST_HISTORY);
	m_pListHistory->EnableNotifyMessage(true);
    AddUiWin(m_pListHistory);
	...
	m_pListHistory->setupButton(&m_BtnDetail);
...
void UI_HistoryWnd::OnMzCommand(WPARAM wParam, LPARAM lParam) {
    ...
    switch (id) {
		...
		{
			setupDetailView(m_pListHistory->GetSelectedIndex());
			break;
		}
        case MZ_IDC_TOOLBAR_ABOUT:
        {
            ...
            if (nIndex == TOOLBARPRO_LEFT_TEXTBUTTON) {
				if(_detailViewMode){
					...
					m_pListHistory->SetVisible(true);
					m_pListHistory->Invalidate();
...
void UI_HistoryWnd::setupDetailView(int idx){
	...
	m_pListHistory->SetVisible(false);
	m_pListHistory->Invalidate();
...
void UI_HistoryWnd::GetHistoryList(){
	...
	m_pListHistory->RemoveAll();

    ListItem li;
    if(pldb->query_size() == 0){
        m_pListHistory->SetTextSize(20);
        li.Text = L"请到论坛下载历史上的今天数据。";
        m_pListHistory->AddItem(li);
        m_pListHistory->SetDataNULL();
    }else{
    ...
    switch (message) {
        case MZ_WM_MOUSE_NOTIFY:
        {
            ...
            if (nID == MZ_IDC_LIST_HISTORY && nNotify == MZ_MN_LBUTTONDOWN) {
                if (!m_pListHistory->IsMouseDownAtScrolling() && 
                    !m_pListHistory->IsMouseMoved() &&
                    !m_pListHistory->isDataNull()) {
                    int nIndex = m_pListHistory->CalcIndexOfPos(x, y);
                    m_pListHistory->SetSelectedIndex(nIndex);
            ...
            if (nID == MZ_IDC_LIST_HISTORY && nNotify == MZ_MN_MOUSEMOVE) {
                m_pListHistory->SetSelectedIndex(-1);
				m_BtnDetail.SetVisible(false);
                m_pListHistory->Invalidate();
                return 0;
            }
